...
def mp_ping(num_procs):
    out_q = Queue()
    procs = []
    for i in range(num_procs):
        # adding each process to the Queue for multiprocessing
        p = Process(target=get_ping, args=(out_q,))
        procs.append(p)
        # starting the process
        p.start()
    results = []
    for i in range(num_procs):
        # add the results of the get_ping() for each process into a results list
...
if __name__ == "__main__":
    ...
    results = mp_ping(num_procs=num_cores)
